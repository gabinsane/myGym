...
from myGym.stable_baselines_mygym.common.tf_util import batch_to_seq, seq_to_batch
from myGym.stable_baselines_mygym.common.tf_layers import conv, linear, conv_to_fc, lstm
from myGym.stable_baselines_mygym.common.distributions import make_proba_dist_type, CategoricalProbabilityDistribution, \
    MultiCategoricalProbabilityDistribution, DiagGaussianProbabilityDistribution, BernoulliProbabilityDistribution
from myGym.stable_baselines_mygym.common.input import observation_input
        ...
        if deterministic:
            ...
            try:
                action, value, neglogp = self.sess.run([self.action, self.value_flat, self.neglogp],
                                                       {self.obs_ph: np.expand_dims(obs, axis=0)})
            except:
                action, value, neglogp = self.sess.run([self.action, self.value_flat, self.neglogp],
                                                       {self.obs_ph: obs})
